...
// Which origin country a port belongs to — the reverse of originPortsFor, used to work out
// a container's origin when the booker is not themselves scoped to a branch.
const countryForOriginPort = (port) => {
  const g = ORIGIN_PORTS.find(x => x.ports.includes(port));
...
module.exports = { SHIPPING_LINES, ORIGIN_PORTS, ORIGIN_COUNTRIES, originPortsFor, countryForOriginPort, DESTINATION_PORTS, PHONE_FORMATS, phoneFormatFor };
