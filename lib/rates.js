...
// Destination charges beyond the all-in container fee. Each is billed only when incurred.
const EXTRA_CHARGES = [
  { key: 'DEMURRAGE_PER_DAY',   label: 'Demurrage (per day)',            unit: 'day' },
  { key: 'DETENTION_PER_DAY',   label: 'Container detention (per day)',  unit: 'day' },
  { key: 'STORAGE_PER_DAY',     label: 'Warehouse storage (per day)',    unit: 'day' },
  { key: 'CUSTOMS_EXAM',        label: 'Customs examination / X-ray',    unit: 'container' },
  { key: 'ALERT_ORDER',         label: 'Alert order / 100% inspection',  unit: 'container' },
  { key: 'REDELIVERY',          label: 'Re-delivery attempt',            unit: 'box' },
  { key: 'REMOTE_AREA',         label: 'Remote-area delivery surcharge', unit: 'box' },
  { key: 'STORAGE_UNCLAIMED',   label: 'Unclaimed-box storage',          unit: 'box' },
  { key: 'DOCUMENTATION',       label: 'Documentation / brokerage',      unit: 'shipment' },
  { key: 'CRANE_LIFT',          label: 'Crane / lift-on lift-off',       unit: 'container' }
];
const EXTRA_KEYS = EXTRA_CHARGES.map(x => x.key);
...
function defaultRateCard() {
  ...
  return {
    ...
    // Extra destination charges that only apply when they actually happen — added as lines
    // on a settlement rather than assumed.
    interbranch_extras: Object.fromEntries(EXTRA_CHARGES.map(x => [x.key, 0])),
    updated_at: null,
    updated_by: null
  };
}
...
function normalizeRateCard(card) {
  ...
  out.interbranch_extras = { ...base.interbranch_extras, ...(card.interbranch_extras || {}) };
  return out;
}
...
// Ocean Priority is a Metro Manila service. The speed comes from delivering straight off the
// container in the city it is stripped in; once a box has to catch a provincial leg or an
// inter-island boat, the promise is one the schedule cannot keep. Offering it anywhere else
// would be selling a delivery date that does not exist.
const PRIORITY_ZONES = ['METRO_MANILA'];
function serviceAvailable(service_level, zone) {
  if (service_level !== 'OCEAN_PRIORITY') return true;
  return PRIORITY_ZONES.includes(zone);
}
// The levels that can actually be sold to a given destination.
function levelsForZone(zone) {
  return OCEAN_LEVELS.concat([AIR_LEVEL]).filter(l => serviceAvailable(l, zone));
}

module.exports = {
  ZONES, ZONE_KEYS, ZONE_LABELS, REGION_ZONE, zoneForRegion,
  PRIORITY_ZONES, serviceAvailable, levelsForZone,
  OCEAN_LEVELS, AIR_LEVEL,
  CONTAINER_SIZES, CONTAINER_SIZE_LABELS, EXTRA_CHARGES, EXTRA_KEYS,
