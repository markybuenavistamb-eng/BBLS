...
function normalizeRateCard(card) {
  ...
  out.interbranch_handling = { ...base.interbranch_handling, ...(card.interbranch_handling || {}) };
  return out;
}

// What head office charges an origin branch for handling one box to a given zone.
function handlingFee(card, zone) {
  const c = normalizeRateCard(card);
  return { amount: num(c.interbranch_handling[zone]), currency: c.currency };
}
...
module.exports = {
  ...
  defaultRateCard, normalizeRateCard, priceBox, emptyBoxPrice, handlingFee
};
