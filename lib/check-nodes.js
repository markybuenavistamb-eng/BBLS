...
async function main() {
  ...
  for (const r of results) {
    ...
    const c = h.checks || {};
    if (!c.database_connected) { console.log(bad('Database not reachable from this deployment.')); problems++; }
    if (!c.node_id_set) {
    ...
    if (!c.sync_secret_set) { console.log(warn('VFIC_SYNC_SECRET not set — replication is off for this node.')); problems++; }
