...
//   Thailand → THB   Cambodia → KHR   Manila head office → PHP
function branchRateCards() {
  ...
  return {
    ...
    // ~71.4 KHR per PHP
    KH_PHNOMPENH: scale(php, 71.4, 'KHR', row(13000, 16000, 19000, 23000))
  };
}
...
// Each branch prices in its own currency.
function branchCurrency(country) {
  return { Thailand: 'THB', Cambodia: 'KHR' }[country] || 'PHP';
}

// What a fresh database is filled with. Set VFIC_SEED on the deployment:
//
//   demo  (default) — the full worked example: customers, shipments, boxes, containers.
//                     Useful for trying the system out, wrong for a live branch.
//   staff           — the staff accounts and settings only, no cargo. What you want when
//                     the system goes live and real intake is about to begin.
//   none            — nothing at all. Only sensible if a database is restored separately,
//                     since with no users nobody can sign in.
//
// This matters more than it looks: every node seeds independently, so demo data seeded on
// three deployments becomes three copies of the same shipments once they replicate to each
// other. Wiping without changing the mode simply recreates them.
const SEED_MODE = () => {
  const m = String(process.env.VFIC_SEED || 'demo').toLowerCase();
  return ['demo', 'staff', 'none'].includes(m) ? m : 'demo';
};

function build(opts = {}) {
  const mode = opts.mode || SEED_MODE();
  const d = {
    seq: {},
    users: [],
    customers: [],
    ...
    settings: {
      ...
      // BSP reference exchange rates, used to state the head-office consolidated books in
      // pesos. The Developer refreshes these from the BSP bulletin in Accounting.
      fx: require('./fx').DEFAULT_FX,
  ...
  // The numbers people quote are banded the same way (see lib/db.js). Every node seeds the
  // same worked example, so numbering it from 1 everywhere is what put two different Thailand
  // shipments on TH-2026-000007 once the nodes replicated.
  const NUMBER_BAND = require('./node').NUMBER_BAND;
  const refCode = (prefix, n) => `${prefix}-${YEAR}-${String(NUMBER_BAND + n).padStart(6, '0')}`;
  ...
  // A live deployment stops here: staff accounts and settings, and not one invented box.
  if (mode === 'staff') return d;
  if (mode === 'none') { d.users = []; return d; }
  ...
  const cStripped = {
    ...
    load_code: 'TH-C1', load_plan_notes: '',
  ...
  const cTransit = {
    ...
    load_code: 'KH-C2', load_plan_notes: '',
    status: 'IN_TRANSIT', created_at: daysAgo(25)
  };
  // Cambodia's counterparts. A container is stuffed at one origin warehouse and carries only
  // boxes received there, so each branch needs its own arrived and in-transit container —
  // otherwise the demo data shows a Bangkok container holding Phnom Penh boxes.
  const cKhStripped = {
    id: nid('container'), container_number: 'CMAU4471902', size: 'C40',
    shipping_line: 'CMA-CGM (The French Line)', vessel_name: 'CMA CGM Mekong', booking_number: 'BKG-88530',
    origin_port: 'Sihanoukville, Cambodia', destination_port: 'Manila International Container Terminal (North)',
    etd: daysAgo(44), eta: daysAgo(13), actual_departure: daysAgo(43), actual_arrival: daysAgo(12),
    load_code: 'KH-C1', load_plan_notes: '',
    status: 'STRIPPED', created_at: daysAgo(58)
  };
  const cThTransit = {
    id: nid('container'), container_number: 'MSCU9938471', size: 'C20',
    shipping_line: 'MSC Mediterranean Shipping Company S.A., Geneva', vessel_name: 'MSC Ilona', booking_number: 'BKG-90402',
    origin_port: 'Laem Chabang, Thailand', destination_port: 'Manila International Container Terminal (North)',
    etd: daysAgo(11), eta: daysAhead(17), actual_departure: daysAgo(10), actual_arrival: null,
    load_code: 'TH-C2', load_plan_notes: '',
    status: 'IN_TRANSIT', created_at: daysAgo(24)
  };
  d.containers.push(cStripped, cKhStripped, cThTransit, cTransit);

  // ---------- trips ----------
  const tripNcr = {
    id: nid('trip'), trip_number: `TRIP-${YEAR}-0001`,
    driver_name: 'Edgar Manalo', driver_contact: '+63 917 555 0400', plate_number: 'NBC 1234',
    trucking_company: 'JRS Co-loader', region: 'NCR', scheduled_date: daysAgo(2).slice(0, 10),
  ...
  const tripSouth = {
    ...
    trucking_company: 'VFIC own truck', region: 'R4A', scheduled_date: daysAhead(1).slice(0, 10),
  ...
  const mkShipment = (sender, opts = {}) => {
    ...
    d.seq[`shipment_number:${code}`] = NUMBER_BAND + shipSeq;
    const s = {
      id: nid('shipment'),
      shipment_number: refCode(code, shipSeq),
      sender_id: sender.id,
      ...
      // Fee and currency follow the origin branch: Thailand bills THB, Cambodia KHR.
      shipping_fee_amount: null, currency: branchCurrency(opts.origin || 'Thailand'),
  ...
  const mkBox = (shipment, idx, receiver, status, opts = {}) => {
    ...
    const b = {
      ...
      container_load_code: opts.container_id === cStripped.id ? 'TH-C1' : opts.container_id === cTransit.id ? 'KH-C1' : null,
      container_box_number: opts.container_id
        ? `${shipment.shipment_number}-${String(idx).padStart(2, '0')}/${opts.container_id === cStripped.id ? 'TH-C1' : 'KH-C1'}`
        : null,
  ...
  const sh1 = mkShipment(sMaria, { created: daysAgo(55) });
  const sh1Items = [
  ...
  const sh2 = mkShipment(sJose, { created: daysAgo(50), origin: 'Cambodia', agent: 'Phnom Penh' });
  ...
  const sh3 = mkShipment(sAna, { created: daysAgo(48), origin: 'Cambodia', agent: 'Phnom Penh' });
  ...
  const sh4 = mkShipment(sPedro, { created: daysAgo(47), origin: 'Thailand', agent: 'Bangkok', paid: false });
  ...
  const sh5 = mkShipment(sMaria, { created: daysAgo(45) });
  ...
  const sh6 = mkShipment(sJose, { created: daysAgo(20), origin: 'Cambodia', agent: 'Phnom Penh' });
  for (let i = 1; i <= 4; i++) mkBox(sh6, i, receivers[i % receivers.length], 'IN_TRANSIT', { age: 20, container_id: cTransit.id });
  const sh7 = mkShipment(sMaria, { created: daysAgo(19) });
  ...
  d.seq['intake_request_code:IR'] = NUMBER_BAND + 1;
  // The two trips above take 0001 and 0002; without this the first trip anyone books mints
  // 0001 again, which is how two different runs ended up sharing a number.
  d.seq.trip_number = 2;
  d.intake_requests.push({
    id: nid('intake_request'),
    reference_code: refCode('IR', 1),
    status: 'PENDING',
    ...
    total_value_php: 18500, currency: branchCurrency('Thailand'), payment_status: 'UNPAID',
    passport_file: null,
    boxes: [
      {
        receiver: {
  ...
  d.box_orders.push({
    id: nid('box_order'), reference_code: refCode('BO', 1), status: 'NEW', submitted_at: daysAgo(1),
    // The branch the order was placed with — a branch only sees its own.
  ...
  d.box_orders.push({
    id: nid('box_order'), reference_code: refCode('BO', 2), status: 'PREPARING', submitted_at: daysAgo(3),
  ...
  // ---------- put every box on a container from its own origin ----------
  // The boxes above were assigned by hand while writing the story, which mixed the two
  // origins onto one container. Reassign each to the container that sails from the country
  // it was received in, in the state its own status implies: still at sea, or arrived.
  {
    const REF = require('./refdata');
    const atSea = (status) => status === 'IN_TRANSIT';
    const pick = (country, status) => {
      const wantSea = atSea(status);
      return d.containers.find(c => REF.countryForOriginPort(c.origin_port) === country
        && (wantSea ? c.status === 'IN_TRANSIT' : c.status === 'STRIPPED')) || null;
    };
    for (const b of d.boxes) {
      if (!b.container_id) continue;
      const sh = d.shipments.find(x => x.id === b.shipment_id);
      const target = sh && pick(sh.origin_country, b.status);
      if (!target) continue;
      b.container_id = target.id;
      b.container_load_code = target.load_code;
      b.container_box_number = `${b.box_number}/${target.load_code}`;
    }
    // Renumber the load sequence within each container so the load plan reads in order.
    for (const c of d.containers) {
      let n = 0;
      for (const b of d.boxes) if (b.container_id === c.id) b.load_sequence = ++n;
    }
  }

  d.seq['box_order_code:BO'] = NUMBER_BAND + 2; // next public order continues from the third in this node's band

  // ---------- head office's own books ----------
  // Manila does not book what the branches bill their senders. Its revenue is the all-in
  // settlement it issues each origin branch per container, and its costs are the local
  // Philippine ones — so both sides of the HQ statement need sample data.
  const ibCard = d.settings.rateCards.HQ_MANILA;
  const mkSettlement = (from, to, container, status, opts = {}) => {
    d.seq.ib_no = (d.seq.ib_no || 0) + 1;
    const amount = ibCard.interbranch_container[container.size] || 48000;
    const inv = {
      id: nid('interbranch_invoice'),
      invoice_number: `IB-${countryCode((require('./branches').byKey(from) || {}).country)}-${YEAR}-${String(d.seq.ib_no).padStart(5, '0')}`,
      from_branch: from, to_branch: to,
      period_from: null, period_to: null,
      currency: ibCard.currency,
      lines: [{
        kind: 'CONTAINER', description: `ALL-IN destination service — container ${container.container_number}`,
        qty: 1, unit_amount: amount, amount
      }],
      total: amount,
      status, notes: opts.notes || '',
      issued_at: daysAgo(opts.issuedDaysAgo != null ? opts.issuedDaysAgo : 30),
      paid_at: status === 'PAID' ? daysAgo(opts.paidDaysAgo != null ? opts.paidDaysAgo : 12) : null,
      settled_reference: status === 'PAID' ? opts.ref || '' : '',
      created_at: daysAgo((opts.issuedDaysAgo != null ? opts.issuedDaysAgo : 30) + 2),
      created_by: admin.name
    };
    d.interbranch_invoices.push(inv);
    return inv;
  };
  // One settlement already paid by Thailand, one still outstanding on Cambodia's container.
  mkSettlement('HQ_MANILA', 'TH_BANGKOK', cStripped, 'PAID', { issuedDaysAgo: 40, paidDaysAgo: 22, ref: 'BDO ref 8841002' });
  mkSettlement('HQ_MANILA', 'KH_PHNOMPENH', cTransit, 'ISSUED', { issuedDaysAgo: 14, notes: 'Due on arrival at Manila' });

  // Local Philippine costs head office actually carries.
  const mkExpense = (category, description, amount, daysAgoSpent, container) => {
    d.expenses.push({
      id: nid('expense'), category, description, amount,
      branch: 'HQ_MANILA', currency: 'PHP',
      container_id: container ? container.id : null,
      spent_at: daysAgo(daysAgoSpent),
      recorded_by: accountant.name, created_at: daysAgo(daysAgoSpent)
    });
  };
  mkExpense('CUSTOMS', 'Customs brokerage and duties — TGHU7654320', 18500, 35, cStripped);
  mkExpense('TRUCKING', 'Port-to-warehouse haulage — Manila South Harbor', 12000, 34, cStripped);
  mkExpense('WAREHOUSE', 'Warehouse rent — monthly', 45000, 30, null);
  mkExpense('TRUCKING', 'NCR delivery runs — fuel and driver', 9800, 6, null);
  mkExpense('OTHER', 'Office utilities and supplies', 6200, 5, null);

  // ---------- price every shipment from its own branch's rate card ----------
  // The fee an agent records must match what the sender is quoted, so the demo data is
  // priced the same way the app prices a real booking: per box, by size and destination
  // zone, in the origin branch's currency.
  {
    const RATES = require('./rates');
    const BRANCH = require('./branches');
    for (const sh of d.shipments) {
      const branchKey = (BRANCH.byCountry(sh.origin_country) || {}).key || 'HQ_MANILA';
      const card = RATES.normalizeRateCard(d.settings.rateCards[branchKey]);
      const total = d.boxes.filter(b => b.shipment_id === sh.id).reduce((sum, b) => {
        const rcv = d.customers.find(c => c.id === b.receiver_id) || {};
        const zone = RATES.zoneForRegion(b.region || rcv.region);
        return sum + RATES.priceBox({ card, service_level: sh.service_level, zone, size_category: b.size_category, weight_kg: b.weight_kg }).amount;
      }, 0);
      sh.shipping_fee_amount = +total.toFixed(2);
      sh.currency = card.currency;
    }
  }
