...
// Example rate card (PHP). Ocean levels are priced per box by size; Express Air per kilo.
// Columns are the four box sizes: Small, Medium, Large, Giga.
...
function build() {
  const d = {
    ...
    settings: {
      ...
      // Starting commercial rates — VFIC edits these in Accounting → Rate Cards.
  ...
  const cTransit = {
    ...
    origin_port: 'Ho Chi Minh City (Cat Lai), Vietnam', destination_port: 'Manila International Container Terminal (North)',
