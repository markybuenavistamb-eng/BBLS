...
const BOX_STATUSES = [
  'CREATED', 'PICKED_UP', 'RECEIVED_BRANCH', 'RECEIVED_ORIGIN', 'LOADED_CONTAINER', 'IN_TRANSIT', 'ARRIVED_PORT',
...
const ORIGIN_SIDE_STATUSES = ['CREATED', 'PICKED_UP', 'RECEIVED_BRANCH', 'RECEIVED_ORIGIN', 'LOADED_CONTAINER'];
...
const TRANSITIONS = {
  CREATED: ['PICKED_UP', 'RECEIVED_BRANCH', 'RECEIVED_ORIGIN'],  // last = dropped straight at the warehouse
  PICKED_UP: ['RECEIVED_BRANCH', 'RECEIVED_ORIGIN'],              // the van reaches one or the other
...
// Who may cancel a box, and when. A cancellation is a booking that will not happen — most
// often the sender changing their mind while the box is still at origin, which is the branch's
// own customer and the branch's own decision. Head office keeps the power everywhere, because
// once a box has sailed the branch is a continent away from the thing being cancelled.
function canCancel(from, actorRole) {
  if (!PRE_DELIVERY.includes(from)) return false;
  const roles = require('./roles');
  if (roles.isAdmin(actorRole)) return true;
  return roles.isBranchAdmin(actorRole) && ORIGIN_SIDE_STATUSES.includes(from);
}

function canTransition(from, to, actorRole) {
  if (to === 'CANCELLED') return canCancel(from, actorRole);
...
module.exports = {
  BOX_STATUSES, FRIENDLY, TRANSITIONS, PRE_DELIVERY, canTransition, canCancel,
