...
const DEFAULT_TEMPLATES = {
  RECEIVED_BRANCH: {
    enabled: false,
    recipients: ['SENDER'],
    body: 'VFIC: Your box {box_number} is with our branch office. Track it anytime: {link}'
  },
  RECEIVED_ORIGIN: {
    enabled: false,
    recipients: ['SENDER'],
    body: 'VFIC: Your box {box_number} reached our warehouse and is waiting for its container. Track: {link}'
  },
  ARRIVED_PORT: {
    enabled: false,
    recipients: ['RECEIVER'],
  ...
  NEARBY: {
    enabled: true,
    recipients: ['RECEIVER'],
    body: 'VFIC: Box {box_number} is nearby and will reach you shortly. Driver: {driver_name} {driver_contact}. Please keep your phone open.'
  },
  OUT_FOR_DELIVERY: {
    enabled: true,
    recipients: ['RECEIVER'],
...
function queueForTrigger(box, triggerKey, extraVars = {}) {
  const d = db.get();
  // Field by field, not template by template: a template saved before the switch existed would
  // otherwise replace the default wholesale, arrive with no switch at all, and be treated as on —
  // which is the one direction this must never fail in.
  const saved = (d.settings.smsTemplates || {})[triggerKey];
  const tpl = DEFAULT_TEMPLATES[triggerKey] ? { ...DEFAULT_TEMPLATES[triggerKey], ...(saved || {}) } : saved;
  // A template that is switched off queues nothing at all, rather than queueing and declining to
  // send: an admin turning it on later should not release a backlog of texts about boxes that
...
async function processOnce() {
  ...
  const pending = d.notifications.filter(n =>
    n.status === 'QUEUED' && n.attempts < 3 &&
    // Unstamped means queued here a moment ago and not yet saved, so it is ours.
    (!n._node || n._node === NODE.NODE_ID));
  let sent = 0, failed = 0;
...
// Send one message straight out, for things that are not about a box — a sign-up code has
// no shipment to hang off. Returns which provider carried it so the caller can tell a real
// delivery from one that only reached the server console.
async function sendNow(phone, message) {
  const provider = getProvider();
  return provider.send(phone, message);
}

// Whether messages actually leave the building. With nothing configured they go to the
// console, which is fine in development and useless as a way to prove someone owns a number.
function deliveryConfigured() {
  return process.env.SMS_PROVIDER === 'semaphore' && !!process.env.SEMAPHORE_API_KEY;
}

module.exports = { queueForTrigger, processOnce, startWorker, DEFAULT_TEMPLATES, REASON_TEXT, renderTemplate, templateVars, sendNow, deliveryConfigured };
