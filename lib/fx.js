// Exchange rates, for reading head office's peso charges in a branch's own money.
//
// Only head office raises inter-branch settlements, and it raises them in pesos. A branch
// acknowledges and pays one, so all it needs is a single rate: PHP into its own currency.
// That makes the whole job PHP→THB for Bangkok and PHP→KHR for Phnom Penh, with PHP→USD
// kept for branches still billing in dollars.
//
// Each branch holds its own rate, taken from the institution it actually banks against:
//   Bangkok    → Bank of Thailand      (quotes the peso directly)
//   Phnom Penh → ACLEDA Bank           (no peso quote, so crossed through the dollar)
//   Manila     → BSP                   (only for the group roll-up; it converts nothing)
//
// Rates are always editable by hand. A refresh that cannot reach its source leaves the
// stored rate untouched and says why, rather than guessing.
//
// Every rate is stored the same way: pesos per one unit of the currency.

const https = require('https');

const BSP_PAGE = 'https://www.bsp.gov.ph/SitePages/Statistics/DailyRERB.aspx';

// Only the money VFIC actually settles in. Head office bills its branches in pesos and
// each branch reads that bill in its own currency, so PHP→THB, PHP→USD and PHP→KHR are the
// whole job. Carrying currencies nobody trades in only invited them to go stale.
const CURRENCIES = ['PHP', 'THB', 'USD', 'KHR'];
...
function get(url, { redirects = 3 } = {}) {
  return new Promise((resolve, reject) => {
    ...
    req.on('timeout', () => req.destroy(new Error('no response within 12s')));
    req.on('error', reject);
  });
}
...
// --- Bank of Thailand ---------------------------------------------------------------
// The published page renders its table in the browser, but the component behind it serves
// the same figures as JSON, which is what this reads. Rates are quoted as THB per one unit
// of the foreign currency, as a buying/selling pair; the mid-point is used, matching ACLEDA.
//
// BOT quotes the peso directly, so no cross is needed: every rate is divided by the peso's
// own quote to turn "THB per unit" into "PHP per unit", which is how the table stores them.
const BOT_PAGE = 'https://www.bot.or.th/en/statistics/exchange-rate.html';
const BOT_DATA = 'https://www.bot.or.th/content/bot/en/statistics/exchange-rate/jcr:content/root/container/statisticstable2.results.level1cache.json';

// BOT does not quote every currency per single unit, and does not always say so. The yen is
// labelled "(100 YEN)" and the rupiah "(1,000 RUPIAH)", but the riel, kip and dong are also
// quoted per 100 with nothing in the name to show it — taking those at face value would put
// a rate out by a factor of a hundred.
//
// So a quote is only accepted when its basis is certain: either the name states the
// multiplier, or the currency is one of the majors BOT quotes per single unit. Anything else
// is skipped and reported, which leaves the rate already on file untouched.
const BOT_PER_UNIT = ['USD', 'EUR', 'GBP', 'AUD', 'CAD', 'CHF', 'SGD', 'HKD', 'NZD', 'PHP',
  'CNY', 'TWD', 'MYR', 'INR', 'AED', 'SAR', 'KWD', 'BHD', 'OMR', 'QAR', 'JOD', 'ILS', 'BND'];

function parseBot(json) {
  const rows = (json && json.responseContent) || [];
  const num = (v) => { const n = Number(String(v == null ? '' : v).replace(/,/g, '')); return isFinite(n) && n > 0 ? n : null; };
  // Mid of transfer-buying and selling; sight-buying is the fallback for thin currencies.
  const mid = (r) => {
    const buy = num(r.buying_transfer) || num(r.buying_sight);
  ...
  const skipped = [];
  for (const r of rows) {
    const code = String(r.currency_id || '').toUpperCase();
    const v = mid(r);
    if (!code || !v) continue;
    // "(100 YEN)" / "(1,000 RUPIAH)" — a stated basis is authoritative.
    const stated = /\(\s*([\d,]+)\s+[A-Za-z]/.exec(String(r.currency_name_eng || ''));
    const per = stated ? Number(stated[1].replace(/,/g, '')) : (BOT_PER_UNIT.includes(code) ? 1 : null);
    if (!per) { skipped.push(code); continue; }
    thbPerUnit[code] = v / per;
  }
  const as_of = rows.length && /^\d{4}-\d{2}-\d{2}$/.test(String(rows[0].period || '')) ? rows[0].period : null;
  return { thbPerUnit, as_of, skipped };
}

async function refreshFromBot() {
  try {
    const raw = (await get(BOT_DATA)).toString('utf8');
    if (!raw.trim().startsWith('{')) {
      return { ok: false, error: 'Bank of Thailand returned a page instead of its rate data — the site may be under maintenance. Try again later or enter the rate by hand.', url: BOT_PAGE };
    }
    const { thbPerUnit, as_of } = parseBot(JSON.parse(raw));
    const thbPerPhp = thbPerUnit.PHP;
    if (!thbPerPhp) {
      return { ok: false, error: 'Read the BOT table but it did not quote the Philippine peso, so the baht cannot be priced against it. Enter the rate by hand.', url: BOT_PAGE };
    }
    // The table stores PHP per one unit, so divide every BOT quote by the peso's own.
    const rates = { PHP: 1, THB: +(1 / thbPerPhp).toFixed(6) };
    for (const [code, thb] of Object.entries(thbPerUnit)) {
      if (code === 'PHP' || !CURRENCIES.includes(code)) continue;
      rates[code] = +(thb / thbPerPhp).toFixed(6);
    }
    return { ok: true, rates, as_of: as_of || new Date().toISOString().slice(0, 10), url: BOT_PAGE };
  } catch (e) {
    return { ok: false, error: 'Could not reach Bank of Thailand: ' + e.message };
  }
}
...
module.exports = {
  BSP_PAGE, ACLEDA_PAGE, BOT_PAGE, BOT_DATA, CURRENCIES, DEFAULT_FX,
  refreshFromAcleda, refreshFromBot, refreshForBranch, parseAcleda, parseBot,
