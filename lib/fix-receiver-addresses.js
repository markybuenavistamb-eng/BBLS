...
function receiverFor(doc, r, want, typed) {
  ...
  // The booking stores the region the way the sender saw it on the form — "Davao Region" — but
  // a customer record holds the PSGC code, and sorting for delivery reads the code. Guessing at
  // REGION.fromProvince (which does not exist) quietly wrote null, and a box with no region
  // cannot be sorted when it reaches Manila.
  const region = REGION.CODES.includes(typed.region) ? typed.region
    : REGION.mapPsgcRegion(typed.region || typed.province || typed.city_municipality);
  const customer = {
