...
const MODULES = [
  ...
  { key: 'driver_passes',    label: 'Driver Passes',      route: '#/driver-passes',    group: 'Operations' },
  { key: 'schedule',         label: 'Schedule',           route: '#/schedule',         group: 'Operations' },
...
// Modules that only exist on the Philippine side, plus the developer's own console. A branch
// never receives, sorts, trucks or returns anything — that happens after its containers land
// in Manila — so these have no meaning in a branch portal and are not a branch admin's to
// grant. Listing them there only invited someone to switch on a module that leads nowhere.
const PH_SIDE_MODULES = ['ph_warehouse', 'trips', 'returns', 'developer'];
const BRANCH_MODULES = MODULES.filter(m => !PH_SIDE_MODULES.includes(m.key));
const modulesForBranchAdmin = () => BRANCH_MODULES.slice();
...
const SHIPPER_MODULES = ['dashboard', 'shipments', 'box_orders', 'boxes', 'containers', 'origin_warehouse', 'driver_passes', 'schedule', 'customers', 'sms', 'reports', 'scan'];
...
const DEFAULTS = {
  ...
  CONSIGNEE_AGENT: ['dashboard', 'shipments', 'box_orders', 'boxes', 'containers', 'ph_warehouse', 'trips', 'driver_passes', 'schedule', 'returns', 'customers', 'sms', 'reports', 'scan'],
...
module.exports = {
  PH_SIDE_MODULES, BRANCH_MODULES, modulesForBranchAdmin, MODULES, MODULE_KEYS, MODULE_LABELS, DEFAULTS, LOCKED, defaultsFor, modulesForRole, matrix, canUseModule };
