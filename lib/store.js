...
// Inspect the *shape* of the configured credentials without revealing them, so a rejected
// key can say which mistake it is. Every finding below is a property of the value (quoted,
// padded, wrong role, wrong project) — never the value itself.
function credentialDiagnosis() {
  const notes = [];
  const quoted = (v) => /^["'].*["']$/.test(v || '');
  const padded = (v) => v != null && v !== String(v).trim();

  if (useSB) {
    const rawKey = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_KEY || '';
    const rawUrl = process.env.SUPABASE_URL || '';
    if (quoted(rawKey)) notes.push('The service_role key is wrapped in quotes — paste the bare value, Vercel does not strip them from a typed field.');
    if (quoted(rawUrl)) notes.push('SUPABASE_URL is wrapped in quotes — paste the bare value.');
    if (padded(rawKey)) notes.push('The service_role key has leading or trailing whitespace.');
    const key = String(rawKey).trim().replace(/^["']|["']$/g, '');
    const parts = key.split('.');
    // Supabase issues two generations of key. Legacy ones are JWTs (anon / service_role);
    // newer projects issue sb_publishable_… and sb_secret_…, which are not JWTs and must not
    // be reported as malformed. Both generations authenticate through the same apikey header.
    if (/^sb_secret_/.test(key)) {
      // Correct kind of key — nothing to say about its shape. A 401 here means the value is
      // stale, belongs to another project, or was revoked.
    } else if (/^sb_publishable_/.test(key)) {
      notes.push('This is the publishable key, which is the replacement for anon. Use the secret key (sb_secret_…) from Project Settings → API Keys — with RLS on and no policy, only the secret key can read the table.');
    } else if (key && parts.length !== 3) {
      notes.push('This is neither a JWT nor an sb_secret_… key. Copy the service_role key (or the secret key on newer projects) from Project Settings → API Keys — not the database password, not the project ref, and not the project URL.');
    } else if (key) {
      try {
        const claims = JSON.parse(Buffer.from(parts[1], 'base64url').toString());
        if (claims.role && claims.role !== 'service_role') {
          notes.push(`The key's role is "${claims.role}", not service_role. With RLS on and no policy, only service_role can read the table.`);
        }
        // A two-project setup makes it easy to give Manila Thailand's key and vice versa.
        const host = (() => { try { return new URL(String(rawUrl).replace(/^["']|["']$/g, '')).host; } catch (e) { return ''; } })();
        const ref = host.split('.')[0];
        if (claims.ref && ref && claims.ref !== ref) {
          notes.push(`The key belongs to Supabase project "${claims.ref}" but SUPABASE_URL points at "${ref}" — the two projects' keys look swapped.`);
        }
        if (claims.exp && claims.exp * 1000 < Date.now()) notes.push('The key has expired.');
      } catch (e) { notes.push('The key could not be decoded — it may be truncated.'); }
    }
    if (!key) notes.push('No service_role key is set on this deployment.');
  } else if (useKV) {
    ...
    if (quoted(rawToken)) notes.push('The REST token is wrapped in quotes — Upstash shows it as TOKEN="…", but paste only what is inside the quotes.');
    if (quoted(rawUrl)) notes.push('The REST URL is wrapped in quotes — paste only what is inside the quotes.');
    if (padded(rawToken)) notes.push('The REST token has leading or trailing whitespace.');
    if (!String(rawToken).trim()) notes.push('No REST token is set on this deployment.');
  }
  return notes;
}
...
async function probe() {
  ...
  try {
    ...
    return { ok: false, ms: Date.now() - started, ...classifyError(e), notes: credentialDiagnosis() };
  }
}

module.exports = { loadDoc, saveDoc, loadDocSync, saveDocSync, useKV, useSB, ephemeral, backend, DATA_DIR, classifyError, probe, credentialDiagnosis };
