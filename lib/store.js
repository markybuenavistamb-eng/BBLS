...
function classifyError(err) {
  ...
  if (/\b401\b|JWT|Invalid API key|invalid signature|invalid_token/i.test(m)) {
    return { reason: 'unauthorized', fix: 'The key was rejected. Use the service_role key from Project Settings → API — not anon, and not the database password. If you rotated it, update the deployment and redeploy.' };
  }
